...
final class OAuth
{
    ...
    public function register(array $input): array
    {
        ...
        $clientId = 'mpauth_' . Util::randomToken(24);
        $stmt = $this->pdo->prepare('INSERT INTO mpauth_oauth_clients(client_id,client_name,redirect_uris) VALUES (?,?,?)');
    ...
    public function beginAuthorization(array $query): void
    {
        ...
        $stmt = $this->pdo->prepare('INSERT INTO mpauth_oauth_requests(request_id,parameters_json,user_id,expires_at) VALUES (?,?,?,DATE_ADD(NOW(6), INTERVAL 30 MINUTE))');
        $user = $this->currentWebUser();
        $stmt->execute([$requestId, Util::jsonEncode($params), $user]);
        if ($user === null) {
            $this->renderLogin($requestId, Config::path('/oauth/authorize'));
            return;
        }
    ...
    public function continueAuthorization(array $post): void
    {
        ...
        self::canonicalResource($params['resource']);
        ...
        if ($action === 'login') {
            ...
            $stmt = $this->pdo->prepare('UPDATE mpauth_oauth_requests SET user_id = ?, expires_at = DATE_ADD(NOW(6), INTERVAL 30 MINUTE) WHERE request_id = ?');
        ...
        Config::assertUser($userId);
        $code = Util::randomToken(32);
        $this->pdo->beginTransaction();
        $consume = $this->pdo->prepare('DELETE FROM mpauth_oauth_requests WHERE request_id=?');
        $consume->execute([$requestId]);
        if ($consume->rowCount() !== 1) { $this->pdo->rollBack(); throw new AppError('invalid_request', 'Request already consumed.'); }
        $stmt = $this->pdo->prepare('INSERT INTO mpauth_oauth_codes(code_hash,client_id,user_id,redirect_uri,resource,scopes_json,code_challenge,expires_at) VALUES (?,?,?,?,?,?,?,DATE_ADD(NOW(6), INTERVAL 5 MINUTE))');
        $stmt->execute([
    ...
    public function revoke(array $post): void
    {
        ...
        $client=Util::requireString($post,'client_id',512);
        $q=$this->pdo->prepare('UPDATE mpauth_oauth_connections SET revoked_at=NOW(6) WHERE id IN (SELECT family_id FROM mpauth_oauth_tokens WHERE token_hash=? AND client_id=? AND resource=?)');
        $q->execute([Util::tokenHash($token),$client,Config::resourceUrl()]);
    }

    public function introspect(array $post): array
    {
        $provided=$_SERVER['PHP_AUTH_PW']??'';
        $client=$_SERVER['PHP_AUTH_USER']??'';
        if ($client!==Config::service()['gateway_client_id'] || !is_string($provided) || !hash_equals(Config::service()['gateway_secret_hash'],hash('sha256',$provided))) {
            header('WWW-Authenticate: Basic realm="MCP introspection"');
            throw new AppError('invalid_client','Gateway authentication required.',401);
        }
        $raw=Util::requireString($post,'token',1024);
        $q=$this->pdo->prepare("SELECT t.*,c.allowed_scopes_json FROM mpauth_oauth_tokens t JOIN mpauth_oauth_connections c ON c.id=t.family_id WHERE t.token_hash=? AND t.token_type='access' AND t.revoked_at IS NULL AND t.expires_at>NOW(6) AND c.revoked_at IS NULL AND t.resource=?");
        $q->execute([Util::tokenHash($raw),Config::resourceUrl()]);$r=$q->fetch(PDO::FETCH_ASSOC);
        if (!$r || !Config::userAllowed((int)$r['user_id'])) return ['active'=>false];
        $scopes=array_values(array_intersect(Config::scopes(),Util::jsonDecode($r['scopes_json']),Util::jsonDecode($r['allowed_scopes_json'])));
        return ['active'=>true,'sub'=>(string)$r['user_id'],'client_id'=>$r['client_id'],'aud'=>$r['resource'],'iss'=>Config::publicUrl(),'exp'=>strtotime($r['expires_at'].' UTC'),'scope'=>implode(' ',$scopes),'connection_id'=>$r['family_id']];
    }

    /** Gateway-only credential delivery. Identity comes exclusively from a validated access token. */
    public function credential(array $post): array
    {
        $identity=$this->introspect($post);
        if (!($identity['active']??false)) return ['active'=>false];
        if (!in_array('mcp:access',explode(' ',$identity['scope']),true)) return ['active'=>false];
        $identity['backend_key']=(new KeyStore($this->pdo))->decrypt((int)$identity['sub']);
        return $identity;
    }
    ...
    public function authenticateAccessToken(?string $requiredScope = null): array
    {
        ...
        $stmt = $this->pdo->prepare(
            ...
            . "FROM mpauth_oauth_tokens t JOIN mpauth_oauth_connections c ON c.id=t.family_id AND c.user_id=t.user_id "
    ...
    public function currentWebUser(): ?int
    {
        ...
        $stmt = $this->pdo->prepare('SELECT user_id FROM mpauth_web_sessions WHERE session_hash=? AND expires_at>NOW(6)');
        $stmt->execute([Util::tokenHash($token)]);
        $user = $stmt->fetchColumn();
        if ($user === false) {
            return null;
        }
        $this->pdo->prepare('UPDATE mpauth_web_sessions SET last_seen_at=NOW(6) WHERE session_hash=?')->execute([Util::tokenHash($token)]);
        return (int) $user;
    }
    ...
    public function renderLogin(string $requestId = '', string $target = ''): void
    {
        $target = $target !== '' ? $target : Config::path('/accounts');
    ...
    private function validateAuthorizationParameters(array $query): array
    {
        ...
        $scopes = $this->parseScopes(is_string($query['scope'] ?? null) ? $query['scope'] : 'mcp:access');
        return [
    ...
    {
        $stmt = $this->pdo->prepare('SELECT client_name,redirect_uris FROM mpauth_oauth_clients WHERE client_id=? AND disabled_at IS NULL');
    ...
    private function fetchClientMetadata(string $url): array
    {
        ...
        $body = '';
        curl_setopt($ch, CURLOPT_WRITEFUNCTION, static function($handle, string $chunk) use (&$body): int { if (strlen($body)+strlen($chunk)>262144) return 0; $body.=$chunk; return strlen($chunk); });
        $ok = curl_exec($ch);
        $status = curl_getinfo($ch, CURLINFO_RESPONSE_CODE);
        $type = (string) curl_getinfo($ch, CURLINFO_CONTENT_TYPE);
        curl_close($ch);
        if ($ok === false || !is_string($body) || $status !== 200 || !str_contains(strtolower($type), 'json') || strlen($body) > 262144) {
    ...
    private function exchangeCode(array $post): array
    {
        ...
        $this->pdo->beginTransaction();
        try {
            $stmt = $this->pdo->prepare('SELECT * FROM mpauth_oauth_codes WHERE code_hash=? FOR UPDATE');
            ...
            if ($row['used_at'] !== null) {
                $this->pdo->prepare('UPDATE mpauth_oauth_connections SET revoked_at=NOW(6) WHERE id=?')->execute([$row['family_id']]);
                $this->pdo->commit();
                throw new AppError('invalid_grant', 'Authorization code reuse detected.');
            }
            ...
            $family = Util::uuid();
            $this->pdo->prepare('UPDATE mpauth_oauth_codes SET used_at=NOW(6),family_id=? WHERE code_hash=?')->execute([$family,Util::tokenHash($code)]);
            $clientNameStmt = $this->pdo->prepare('SELECT client_name FROM mpauth_oauth_clients WHERE client_id=?');
    ...
    private function refresh(array $post): array
    {
        ...
        try {
            ...
            $stmt = $this->pdo->prepare("SELECT * FROM mpauth_oauth_tokens WHERE token_hash=? AND token_type='refresh'");
            ...
            $stmt = $this->pdo->prepare("SELECT * FROM mpauth_oauth_tokens WHERE token_hash=? AND token_type='refresh' FOR UPDATE");
            $stmt->execute([$tokenHash]);
            $row = $stmt->fetch(PDO::FETCH_ASSOC);
            if (!is_array($row)) {
                throw new AppError('invalid_grant', 'Refresh token is invalid or expired.', 400);
            }
            if (!hash_equals((string)$row['client_id'],$clientId) || !hash_equals((string)$row['resource'],$resource)) {
                throw new AppError('invalid_grant','Wrong client or resource.');
            }
            Config::assertUser((int)$row['user_id']);
            if ($row['used_at'] !== null) {
            ...
            $this->pdo->prepare('UPDATE mpauth_oauth_tokens SET used_at=NOW(6),revoked_at=NOW(6) WHERE token_hash=?')->execute([$tokenHash]);
            if (isset($post['scope'])) {
                if (!is_string($post['scope'])) {
                    throw new AppError('invalid_scope', 'scope must be a string.', 400);
                }
                $scopes = array_values(array_intersect($scopes, Util::jsonDecode((string)$row['scopes_json'])));
    ...
    private function createWebSession(int $userId): void
    {
        ...
        $stmt = $this->pdo->prepare('INSERT INTO mpauth_web_sessions(session_hash,user_id,csrf_token,expires_at) VALUES (?,?,?,DATE_ADD(NOW(6), INTERVAL 12 HOUR))');
    ...
    private static function canonicalResource(mixed $value): string
    {
        ...
        if (!is_array($parts) || !isset($parts['scheme'], $parts['host']) || isset($parts['query']) || isset($parts['fragment']) || isset($parts['user']) || isset($parts['pass'])) {
    ...
    {
        if (!is_string($uri) || strlen($uri) > 2048 || str_contains($uri, '#') || preg_match('/[\x00-\x20]/',$uri)) {
            throw new AppError('invalid_client_metadata', 'Invalid redirect URI.', 400);
        }
        $parts = parse_url($uri);
        $scheme = is_array($parts) ? strtolower((string) ($parts['scheme'] ?? '')) : '';
        $host = is_array($parts) ? strtolower((string) ($parts['host'] ?? '')) : '';
        if (!is_array($parts) || $host === '' || isset($parts['user']) || isset($parts['pass'])) throw new AppError('invalid_client_metadata','Invalid redirect URI.');
    ...
    {
        $stmt = $this->pdo->prepare('SELECT * FROM mpauth_oauth_requests WHERE request_id=? AND expires_at>NOW(6)');
        $stmt->execute([$id]);
        $row = $stmt->fetch(PDO::FETCH_ASSOC);
        if (!is_array($row)) {
    ...
    {
        $this->pdo->prepare('DELETE FROM mpauth_oauth_requests WHERE request_id=?')->execute([$id]);
    }
    ...
    {
        $scopes = '<p>Dienst: <strong>' . self::h(Config::service()['name']) . '</strong></p><p>' . self::h(Config::resourceUrl()) . '</p><ul><li>' . implode('</li><li>', array_map([self::class, 'h'], $params['scopes'])) . '</li></ul>';
    ...
    public function html(string $title, string $body, ?string $formAction = null): void
    {
        ...
        echo '<!doctype html><html lang="de"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>' . self::h($title) . '</title><style>*{box-sizing:border-box}html{overflow-wrap:anywhere}body{font:16px/1.5 system-ui;max-width:760px;min-width:0;margin:3rem auto;padding:0 1rem;color:#18212b}h1{font-size:clamp(1.75rem,8vw,2rem);line-height:1.2}label{display:block;margin:1rem 0}input:not([type=checkbox]){display:block;width:100%;min-width:0;min-height:44px;margin-top:.3rem;padding:.6rem;font:inherit}input[type=checkbox]{margin-right:.4rem}button{min-height:44px;padding:.7rem 1rem;margin:.5rem .5rem .5rem 0;font:inherit}table{display:block;width:100%;overflow-x:auto;border-collapse:collapse}th,td{text-align:left;padding:.55rem;border-bottom:1px solid #d8dee4}.connection{min-width:0;border:1px solid #d8dee4;border-radius:.5rem;padding:1rem;margin:1rem 0}.permission{margin:.55rem 0}.muted{color:#66717d;font-size:.9rem}fieldset{min-width:0;border:0;padding:0;margin:1rem 0}legend{font-weight:600}@media(max-width:480px){body{margin:1.5rem auto;padding:0 .875rem}.connection{padding:.875rem}form button{display:block;width:100%;margin:.5rem 0}}</style><body>' . $body . '</body></html>';
    }
