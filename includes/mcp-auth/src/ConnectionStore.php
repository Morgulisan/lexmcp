...
final class ConnectionStore
{
    ...
    public function create(int $userId, string $id, string $clientId, string $name, array $requestedScopes): void
    {
        ...
        $stmt = $this->pdo->prepare('INSERT INTO mpauth_oauth_connections(id,user_id,client_id,name,requested_scopes_json,allowed_scopes_json) VALUES (?,?,?,?,?,?)');
    ...
    public function listForUser(int $userId): array
    {
        $stmt = $this->pdo->prepare(
            ...
            . 'FROM mpauth_oauth_connections c LEFT JOIN mpauth_oauth_clients o ON o.client_id=c.client_id '
    ...
    public function update(int $userId, string $id, string $name, array $allowedScopes): void
    {
        ...
        try {
            $stmt = $this->pdo->prepare('UPDATE mpauth_oauth_connections SET name=?,allowed_scopes_json=? WHERE id=? AND user_id=? AND revoked_at IS NULL');
            $stmt->execute([$name, $scopesJson, $id, $userId]);
            if ($stmt->rowCount() === 0) {
                $check = $this->pdo->prepare('SELECT 1 FROM mpauth_oauth_connections WHERE id=? AND user_id=? AND revoked_at IS NULL FOR UPDATE');
            ...
            $tokens = $this->pdo->prepare('UPDATE mpauth_oauth_tokens SET scopes_json=? WHERE family_id=? AND user_id=? AND revoked_at IS NULL');
    ...
    {
        $sql = 'SELECT allowed_scopes_json FROM mpauth_oauth_connections WHERE id=? AND user_id=? AND revoked_at IS NULL' . ($lock ? ' FOR UPDATE' : '');
