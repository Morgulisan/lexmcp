...
final class OAuth
{
    ...
    public function beginAuthorization(array $query): void
    {
        ...
        $stmt = $this->pdo->prepare('INSERT INTO lxmcp_oauth_requests(request_id,parameters_json,user_id,expires_at) VALUES (?,?,?,DATE_ADD(NOW(6), INTERVAL 10 MINUTE))');
    ...
    public function continueAuthorization(array $post): void
    {
        ...
        if ($action === 'login') {
            ...
            $stmt = $this->pdo->prepare('UPDATE lxmcp_oauth_requests SET user_id = ? WHERE request_id = ?');
    ...
    private function validateAuthorizationParameters(array $query): array
    {
        ...
        Util::assertKeys($query, $allowed, 'oauth_authorize');
        ...
        $resource = Util::requireString($query, 'resource', 2048);
        if (!hash_equals(Config::resourceUrl(), $resource)) {
    ...
    {
        Util::assertKeys($post, ['grant_type', 'code', 'redirect_uri', 'client_id', 'code_verifier', 'resource']);
        ...
        $resource = Util::requireString($post, 'resource', 2048);
    ...
    {
        Util::assertKeys($post, ['grant_type', 'refresh_token', 'client_id', 'resource', 'scope']);
    ...
    private function renderConsent(string $requestId, array $params): void
    {
        ...
        $this->html('Zugriff erlauben', $body);
    }

    public function html(string $title, string $body): void
    {
